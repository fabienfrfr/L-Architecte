...
def _(df_graph, go):
    def plot_architect_graph(df):
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(
                size=12, color=df['cluster'], colorscale='Viridis',
        ...
        fig.update_layout(title="TheArchitect: Multimodal Cluster Graph", showlegend=False)
        return fig
...
def _(subprocess, time):
    def run_infra():
        ...
        subprocess.Popen(
            ["kubectl", "port-forward", "svc/arangodb", "8529:8529", "-n", "agentic-architect"],
        ...
        for _, row in df.iterrows():
            for rel in row['relationships']:
                edges.append({
                    ...
                    "weight": rel['weight'],
                    "type": "bridge" if row['cluster'] != rel['target_cluster'] else "internal"
                })

        rel_coll.import_bulk(edges, overwrite=True)
        print(f"🚀 Successfully synced {len(nodes)} nodes and {len(edges)} edges to ArangoDB.")
